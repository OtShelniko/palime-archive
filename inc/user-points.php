...
// 🔴 Ценные действия
define( 'PALIME_XP_LONGREAD',      10 );   // Дочитал лонгрид (время >2 мин на статье)
...
function palime_add_points( $user_id, $amount, $reason = '', $category = 'base', $ref_id = 0 ) {
...
add_action( 'comment_post', function( $comment_id, $approved ) {
    ...
    if ( $comment && $comment->comment_parent > 0 ) {
        palime_add_points( $user_id, PALIME_XP_REPLY, 'Ответ на комментарий', 'social', $comment_id );
    } else {
        palime_add_points( $user_id, PALIME_XP_COMMENT, 'Комментарий', 'social', $comment_id );
    }

    palime_check_achievements( $user_id );
}, 10, 2 );

// Откат XP при удалении/спаме комментария
add_action( 'transition_comment_status', function( $new_status, $old_status, $comment ) {
    if ( ! $comment || ! $comment->user_id ) return;

    // Если комментарий перешёл в спам или удалён — отнимаем XP
    if ( in_array( $new_status, [ 'spam', 'trash' ], true ) && $old_status === 'approved' ) {
        $user_id = (int) $comment->user_id;
        $rollback = $comment->comment_parent > 0 ? PALIME_XP_REPLY : PALIME_XP_COMMENT;

        $current = (int) get_user_meta( $user_id, 'palime_points', true );
        $new     = max( 0, $current - $rollback );
        update_user_meta( $user_id, 'palime_points', $new );

        $log   = get_user_meta( $user_id, 'palime_points_log', true ) ?: [];
        $log[] = [
            'amount'   => -$rollback,
            'reason'   => 'Откат: комментарий удалён/спам',
            'category' => 'rollback',
            'date'     => current_time( 'mysql' ),
        ];
        if ( count( $log ) > 200 ) {
...
add_action( 'wp_ajax_palime_track_read', function() {
    ...
    if ( $scroll_pct >= 70 ) {
        ...
        palime_add_points( $user_id, PALIME_XP_READ_ARTICLE, 'Прочитал статью', 'base', $article_id );
        $points_awarded += PALIME_XP_READ_ARTICLE;

        // Лонгрид бонус: время > 2 минут на статье
        if ( $time_spent >= 120 ) {
            palime_add_points( $user_id, PALIME_XP_LONGREAD, 'Дочитал лонгрид', 'valuable', $article_id );
...
function palime_get_achievements() {
    return [
        ...
        'save_10' => [
            'name'        => 'Библиотекарь',
            'description' => 'Сохраните 10 материалов',
            'icon'        => '12',
            'bonus'       => 15,
            'rarity'      => 'common',
        ],
        ...
        'longread_master' => [
            'name'        => 'Глубоководный',
            'description' => 'Дочитайте 10 лонгридов (>2 мин каждый)',
            'icon'        => '18',
            'bonus'       => 80,
            'rarity'      => 'rare',
        ],
        ...
        // ── Доступ
        'first_gated_article' => [
            'name'        => 'За закрытой дверью',
            'description' => 'Откройте первый материал, доступный по уровню',
            'icon'        => '22',
            'bonus'       => 5,
            'rarity'      => 'common',
        ],
        ...
        'level_3' => [
            'name'        => 'Архивист',
            'description' => 'Достигните уровня Архивист',
            'icon'        => '23',
            'bonus'       => 25,
            'rarity'      => 'common',
        ],
...
function palime_check_achievements( $user_id ) {
    ...
    $checks = [
        ...
        // Доступ
        'first_gated_article' => (bool) get_user_meta( $user_id, 'palime_opened_gated', true ),

        // Уровневые
        'level_3'        => ( $level['number'] ?? 0 ) >= 3,
